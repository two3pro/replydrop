...
- 评论奖励、10U、留言征集、改名征集、抽奖中奖类互动诱导进入 `blocked`
- 追高买入、转多、订阅实时分析、交易信号/带单/合约等投资订阅诱导进入 `blocked`
- 人工模式仍保留流量优先和风险标签，不替用户决定是否值得回

## 0.2.226

- AI 自动发送链路新增 `auto_safe` / `human_review` / `blocked` 三层，只有 `auto_safe` 会进入 executor `candidates`
- 自动发送池剔除同轮/近期同作者重复命中，避免一轮里连续回复同一个作者
- 短文本指代型视频/梗图在没有 `media.summary` 时进入人工复核，不再仅凭媒体 URL 当作已理解
- BTC/合约/赌场/方向性行情叙事、出入金/不给提/资金安全/收 U、官方酒类促销联名等不再进入自动发送池
- 人工写稿和水滴曝光发现继续保留 0.2.225 的流量优先口径，风险只提示不替用户决定
