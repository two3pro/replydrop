...
当前 Chrome / Brave 运行时包和 Safari 源码公开包都已同步到 `0.2.285`。`0.2.285` 不是只改商店口径，而是把今天 Windows 那边继续补进去、真正会影响商店版用户发送行为的 `content.js` 修复正式并回主线：detail 路线不再反复打错回复层，活动 handoff 不会再把当前线程页硬刷新，线程页内联回复框能更稳定激活，文本注入也更偏“原生插字优先”，减少草稿重影和错层发送。

`0.2.285` 的重点是把这批真实发送修复正式带进公开包，而不是让 `0.2.284` 和 Windows 实测版继续分叉。当前 Chrome / Brave 安装包名为 `replydrop-p2.285.zip`，Safari 源码公开包名为 `replydrop-safari-open-source-0.2.285.zip`。
...
- Chrome / Brave 运行时包：[replydrop-p2.285.zip](./downloads/replydrop-p2.285.zip)
...
- Safari for macOS 源码包：[replydrop-safari-open-source-0.2.285.zip](./downloads/replydrop-safari-open-source-0.2.285.zip)
  - 内含 Safari 扩展源码、Xcode 工程、MIT 许可证和安装说明
  - 这是“源码公开 + 本地自签名安装”包，不提供官方签名安装 app
  - 需要你自己的 Apple ID / Team 在本机签名，具体步骤见包内 `INSTALL.md`
- 版本说明：
  - Chrome / Brave 当前公开基线版本已同步到 `0.2.285`
  - Safari 源码公开包当前也已同步到 `0.2.285`
