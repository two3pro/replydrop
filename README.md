...
当前 Chrome / Brave 公开包继续沿用 Safari `0.2.249` 的共享 runtime 基线；`0.2.250 / 0.2.251` 已把首页 timing/traffic 调优收紧到 traffic-first 正轨，`0.2.252` 把 reply ledger / pickup performance 跟踪链补完整，`0.2.253` 则继续把 homepage traffic gating 拉回人工实盘节奏，并补上 `getReplyPerformanceReport({ todayOnly: true })` 的轻量导出入口。
...
- Chrome / Brave 运行时包：[replydrop-p2.253.zip](./downloads/replydrop-p2.253.zip)
...
- 版本说明：
  - Chrome / Brave 当前公开基线版本已同步到 `0.2.253`
